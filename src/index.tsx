        ...
        <section class="grid gap-6 lg:grid-cols-2">
          <div class="rounded-[2.5rem] border border-white/10 bg-white/5 p-6 backdrop-blur-xl sm:p-8">
            ...
            <div class="max-h-[420px] space-y-3 overflow-y-auto pr-1">
              {posts.map(post => <div class="flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/40 p-4"><div class="min-w-0 flex-1"><p class="truncate text-sm font-bold text-slate-200">{post.title}</p><div class="mt-2 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest"><span class={post.status === 'published' ? 'text-green-400' : 'text-slate-500'}>{post.status === 'published' ? 'Published' : 'Draft'}</span>{post.category && <span class="truncate text-slate-600">• {post.category}</span>}</div></div><a href={`/admin/blog/edit/${post.id}`} class="shrink-0 rounded-lg border border-red-500/30 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-red-300 transition-colors hover:bg-red-500/10 hover:text-white">Edit</a></div>)}
          ...
          <div class="rounded-[2.5rem] border border-white/10 bg-white/5 p-6 backdrop-blur-xl sm:p-8">
            ...
            <div class="max-h-[420px] space-y-3 overflow-y-auto pr-1">
              {projects.map(project => <div class="flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/40 p-4"><div class="min-w-0 flex-1"><p class="truncate text-sm font-bold text-slate-200">{project.title}</p><p class="mt-2 truncate text-[10px] font-black uppercase tracking-widest text-slate-500">{project.slug || 'Tanpa slug'}</p></div><a href={`/admin/projects/edit/${project.id}`} class="shrink-0 rounded-lg border border-red-500/30 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-red-300 transition-colors hover:bg-red-500/10 hover:text-white">Edit</a></div>)}
